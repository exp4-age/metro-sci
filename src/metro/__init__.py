...
def init_core():
    ...
    if sys.version_info[:2] < (3, 3):
        globals()["die"](
            "Requires python version >= 3.3 (found {0})".format(
    ...
    try:
        ...
        globals()["die"](
            "An essential dependency ({0}) could not be "
    ...
    if not load_GUI:
        ...
        globals().update({"QtGui": QtGui, "QtWidgets": QtWidgets, "QtUic": QtUic})
...
def init_gui():
    ...
    def die(msg):
        ...
        button = tkinter.Button(frame, text="Close", command=lambda: root.quit())
        button.grid(pady=5)
    ...
    try:
        ...
        die(
            "An essential dependency ({0}) could not be imported and is "
            "probably missing".format(str(e)[str(e)[:-1].rfind("'") + 1 : -1])
        )

    globals().update(
        {"QtGui": QtGui, "QtWidgets": QtWidgets, "QtUic": QtUic, "die": die}
    )
