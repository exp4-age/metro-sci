...
class ConfigMeasurementDialog(QtWidgets.QDialog):
    class OperatorItem(QtWidgets.QTreeWidgetItem):
        ...
        def configure(self, op_name=None, initial_values={}, show_dialog=True):
            ...
            try:
                dialog_class = getattr(ConfigMeasurementDialog, op_name + "Dialog")
            except AttributeError:
                ...
                item_str = "{0}: {1} ({2})".format(
                    self.op_key.capitalize(), op_name, diag.arg_string()
                )
    ...
    class ScansetItem(QtWidgets.QTreeWidgetItem):
        ...
        def getOperators(self):
            return (self.point_item.getOperator(), self.scan_item.getOperator())
        ...
        def operatorText(self, idx):
            return "{0}/{1}\n{0}/{2}".format(
                idx, self.child(0).text(0), self.child(1).text(0)
            )
    ...
    class FixedPointsDialog(QtWidgets.QDialog):
        ...
        def __init__(self, initial_values):
            ...
            with importlib.resources.path(
                __package__, "measure_fixedpoints.ui"
            ) as fspath:
        ...
        def _addValues(self, values):
            if self.checkApplyFunction.isChecked():
                try:
                    ...
                    metro.app.showException(
                        "An error occured when compiling the custom function:", e
                    )
            else:
                func = self._idn
        ...
        def on_buttonSingleValue_clicked(self):
            try:
                ...
                metro.app.showError(
                    ...
                    "Unable to convert to floating point number.",
                    details=e,
                )
            else:
        ...
        def on_buttonSequence_clicked(self):
            ...
            self._addValues([seq_start + i * seq_step for i in range(seq_count)])
        ...
        def on_buttonBox_accepted(self):
            ...
            self.points = [
                float(self.listPoints.item(i).text())
                for i in range(self.listPoints.count())
            ]
    ...
    def __init__(self):
        ...
        with importlib.resources.path(__package__, "measure_config.ui") as fspath:
            QtUic.loadUi(fspath, self)
    ...
    def _addScanset(self):
        ...
        scanset_item = ConfigMeasurementDialog.ScansetItem(point_item, scan_item)
    ...
    def loadMacro(self, state):
        if isinstance(state, str):
            state = self.listMacros.findItems(state, QtCore.Qt.MatchExactly)[0].data(
                QtCore.Qt.UserRole
            )
    ...
    def getOperators(self):
        if self.treeMeas.topLevelItemCount() > 4:
            ...
            for item_idx in range(3, self.treeMeas.topLevelItemCount()):
                operators.append(self.treeMeas.topLevelItem(item_idx).getOperators())
    ...
    def on_treeMeas_customContextMenuRequested(self, pos):
        ...
        if item is not None:
            if isinstance(item, ConfigMeasurementDialog.OperatorItem):
                ...
                self.actionDownScanset.setEnabled(scanset_idx < scanset_count - 1)
    ...
    def on_actionAddMacro_triggered(self, checked):
        value, success = QtWidgets.QInputDialog.getItem(
            self,
            "Add macro - Metro",
            "Please enter a name for this macro",
            [""]
            + [
