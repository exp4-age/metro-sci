...
class SaveProfileDialog(QtWidgets.QDialog):
    def __init__(self, profiles):
        ...
        with importlib.resources.path(__package__, "profiles_save.ui") as fspath:
        ...
        device_list = sorted(metro.getAllDevices(), key=lambda x: x.getDeviceName())

        for device in device_list:
            device_item = QtWidgets.QListWidgetItem(
                device.getDeviceName(), self.listDevices
            )
        ...
        for channel in channels_list:
            if not (hasattr(channel, "_custom") or hasattr(channel, "_replayed")):
                continue

            channel_item = QtWidgets.QListWidgetItem(channel.name, self.listChannels)
    ...
    def getPath(self):
        return "{0}/{1}.json".format(metro.PROFILE_PATH, self.editName.currentText())
    ...
    def saveAsProfile(self):
        ...
        metro.app.last_used_profile = [
            self.editName.currentText(),
            self.getPath(),
            profile,
        ]
    ...
    def on_buttonBox_accepted(self):
        if not self.editName.currentText():
            QtWidgets.QMessageBox.critical(
                self, self.windowTitle(), "Please enter a profile name."
            )
            return

        if self.editName.currentText().startswith("_"):
            QtWidgets.QMessageBox.critical(
                self, self.windowTitle(), "Profiles may not begin with two underscores."
            )
