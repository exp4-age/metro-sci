...
class LinksLabel(QtWidgets.QLabel):
    ...
    def formatLink(
        self, link, color="#0057AE", bold=False, italic=False, underlined=True
    ):
        css = (
            "color: {0}; font-weight: {1}; font-style: {2}; text-decoration: {3}"
        ).format(
            color,
        ...
        html = '<a href="{0}" style="{1}">{2}</a>'.format(link, css, self.labels[link])
...
class ChannelLinksLabel(LinksLabel):
    ...
    def addChannel(self, channel, label=None, entry_point=None, args={}, **kwargs):
        if label is None:
            label = channel.name
    ...
    def on_linkActivated(self, link):
        try:
            ...
            metro.app.showError(
                ...
                "No channel known with label {0}".format(link),
                e,
            )
        else:
            metro.app.createDisplayDevice(
                metro.getChannel(entry[0]), entry_point=entry[1], args=entry[2]
            )
...
class ChannelLinkMenu(LabelableMenu):
    ...
    def buildForChannel(self, channel, entry_point=None, args={}):
        ...
        try:
            self.addMenu(ChannelLinkMenu.menuDisplayBy).setText("Display by...")
        except AttributeError:
        ...
        self.addAction("Clear current step").setData("__clear__")
    ...
    def on_triggered(self, action):
        ...
        if name is None:
            ...
            value, success = metro.QtWidgets.QInputDialog.getText(
                None,
                f"Duplicate channel - {metro.WINDOW_TITLE}",
            ...
            res = QtWidgets.QMessageBox.warning(
                self,
                f"Reset channel - {metro.WINDOW_TITLE}",
            ...
            metro.app.createDisplayDevice(
                channel, name, show_dialog=True, args=self.args
            )
